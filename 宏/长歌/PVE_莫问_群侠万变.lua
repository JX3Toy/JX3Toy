--[[ 奇穴:[号钟][飞帆][弦风][流照][豪情][师襄][知止][刻梦][书离][云汉][参连][正律和鸣]
秘籍:
宫 2读条 1伤害 1会心
商 2会心 2伤害
徵 2会心 2伤害
...
function Main(g_player)
	if casting("变宫") and castleft() < 0.13 then
		settimer("变宫读条结束")
	end
	...
	--徵读条 + 羽 + 变宫读条 时间
	local nTime = casttime("徵") + cdinterval(16) + casttime("变宫") + 0.5
	...
	if buff("9319") then
		...
		--进入正律合奏
		if fight() and v["目标血量较多"] and dis() < 20 and nobuff("正律和鸣|知音妙意") then
			if cdtime("正律和鸣") < 0.5 then
				f["孤影化双"]()
				if v["孤影CD"] > 1 then
					f["高山流水主动"]()
				end
		...
		if fight() and gettimer("孤影化双") > 0.5 then
			--if v["目标商时间"] <= casttime("变宫") or v["曲风层数"] == 2 or v["曲风层数"] == 6 then
			if v["目标商时间"] <= casttime("变宫") or v["曲风层数"] == 2 then
				CastX("商")
			end

			--if v["目标角时间"] <= casttime("变宫") or v["曲风层数"] == 3 or v["曲风层数"] == 7 then
			if v["目标角时间"] <= casttime("变宫") or v["曲风层数"] == 3 then
				CastX("角")
			end
...
f["孤影化双回"] = function()
	if casting("徵|变徵") and bufftime("孤影化双") < 0.5 then	--徵读条中时间快到了，打断
		fcast(14162)
		return
	end
	if bufftime("孤影化双") <= casttime("变宫") then	--小于一个变宫读条
		cast(14162)
		return
	end
...
function PrintInfo(s)
	local szinfo = "曲风层数:"..v["曲风层数"]..", 目标商:"..v["目标商时间"]..", 目标角:"..v["目标角时间"]..", 横影充能:"..cn("疏影横斜")..", "..cntime("疏影横斜", true)..", 徵充能:"..cn("徵")..", 羽充能"..cn("羽")..", 正律CD:"..v["正律CD"]
	if s then
		szinfo = s..szinfo
	end
	print(szinfo)
