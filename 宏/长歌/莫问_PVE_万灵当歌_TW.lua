--[[ 奇穴: [飛帆][明津][弦風][流照][豪情][師襄][知止][刻夢][爭鳴][雲漢][參連][正律和鳴]
秘籍:
宫 2读条 1伤害 1会心
商 2会心 2伤害
徵 2会心 2伤害
...
function Main(g_player)
	...
	if casting("宮|變宮") and castleft() < 0.13 then
		settimer("宫读条结束")
	end
	...
	--没放技能记录信息，用于排查问题
	if fight() and rela("敌对") and dis() < 20 and face() < 90 and cdleft(16) <= 0 and castleft() <= 0 and gettimer("宮") > 0.5 and gettimer("變宮") > 0.5 and gettimer("徵") > 0.5 and gettimer("變徵") > 0.5 and gettimer("徵读条结束") > 0.5 and state("站立") then
		PrintInfo("---------- 没放技能")
	end
end
...
f["高山流水曲"] = function()
	--进入正律合奏, 孤影 -> 高山主动 -> 正律
	if fight() and v["目标血量较多"] and dis() < 20 and nobuff("正律和鳴") then	--v["曲风层数"] <= 0
...
f["阳春白雪曲"] = function()
	if v["目标血量较多"] and dis() < 20 and v["正律CD"] < v["GCD间隔"] and nobuff("正律和鳴") and v["徵充能次数"] > 0  then	--切高山, 打正律
		f["切换到高山流水"]()
	end
...
f["孤影化双回"] = function()
	if casting("徵|變徵") and bufftime("孤影化雙") < 0.5 then	--徵读条中 时间快到了，打断
		if fcast(14162) then
			settimer("孤影化双回")
		end
		return
	...
	if bufftime("孤影化雙") <= v["宫读条时间"] then	--小于一个变宫读条
		if cast(14162) then
			settimer("孤影化双回")
		end
		return
	...
	if scdtime(14229) > bufftime("孤影化雙") and v["正律CD"] > bufftime("孤影化雙") then		--高山主动和正律用过了
...
f["疏影横斜"] = function()
	if rela("敌对") and v["影子数量"] < 6 then
		local bCast = false
		if gettimer("孤影化双回") > 0.5 and bufftime("孤影化雙") > 0 then	--孤影中
			bCast = true
		end
		if buff("正律和鳴") then	--正律中
			bCast = true
		end
