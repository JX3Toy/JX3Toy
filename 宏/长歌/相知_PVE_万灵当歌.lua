--[[ 奇穴: [蔚风][秋鸿][争簇][绿绮 或 一指回鸾][谪仙][音尘][共潮生][行云][流霜][不器][掷杯][无尽藏]
秘籍:
宫  2读条 1会心 1疗效
商  2疗效 2会心
徵  2会心 2疗效
...
function Main()
	...
	--记录读条结束
	if casting("宫") and castleft() < 0.13 then
		settimer("宫读条结束")
	end
	...
	--对目标放影子
	if v["影子数量"] < 6 and fight() and rela("敌对") and cn("疏影横斜") > 2 then
		cast("疏影横斜")
	end
	...
	--宫
	if v["治疗目标血量"] < 0.9 and gettimer("宫读条结束") > 0.5 and xbufftime("音尘", v["治疗目标"], id()) < 2 then
		CastX("宫", bBreak)
	end
	...
	if fight() and rela("敌对") then
		...
		if v["目标的目标"] ~= 0 then
			--音尘
			if gettimer("宫读条结束") > 0.5 and xbufftime("音尘", v["目标的目标"], id()) < 2 then
				xcast("宫", v["目标的目标"], bBreak)
			end
			--hot
	...
	--脱战救人
	if casting("歌尽影生") and castleft() < 0.13 then
		settimer("歌尽读条结束")
	end
	if nofight() and gettimer("歌尽读条结束") > 1 then
		xcast("歌尽影生", party("有状态:重伤", "距离<20", "视线可达"))
	end

	--给T上hot
	xcast("角", party("没状态:重伤", "距离<20", "内功:洗髓经|铁牢律|明尊琉璃体|铁骨衣", "视线可达", "我的buff时间:角<-1"))
	xcast("商", party("没状态:重伤", "距离<20", "内功:洗髓经|铁牢律|明尊琉璃体|铁骨衣", "视线可达", "我的buff时间:商<-1"))
