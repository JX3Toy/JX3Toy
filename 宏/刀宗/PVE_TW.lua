--[[
奇穴: [淵衝][聚疏][潰延][放皓][威聲][觀釁][界破][長溯][周流][強膂][流嵐][截轅]
秘籍: 优先点效果和距离
行云  1会心 2伤害 1距离
停云  1伤害 1距离 2调息
决云  2伤害 2会心
...
function Main(g_player)
	...
	--等待单双切换
	if gettimer("斷雲勢") < 0.3 or gettimer("孤鋒破浪") < 0.3 or gettimer("滅影追風") < 0.3 then
		--PrintInfo("----------等待单双切换, ")
		return
	end
	...
	--等待驰风释放
	if gettimer("馳風八步") < 0.3 or gettimer("释放驰风八步") < 0.13 then
		--PrintInfo("----------等待驰风释放, ")
		return
	end
	...
	--驰风八步
	if miji(32140, "《遊風步·馳風八步》人偶圖殘頁") and nobuff("23953") then	--有秘籍 没二段
		v["打驰风"] = false
...
f["单手"] = function()
	...
	--断云势
	if rela("敌对") and dis() < range("斷雲勢") and face() < 90 then
		if CastX("斷雲勢") then
			return
		end
	end
...
f["找身形"] = function()
	...
	if v["识破时间"] < 0 and v["身形数量"] > 0 and gettimer("馳風八步") > 0.5 and gettimer("释放驰风八步") > 1 then	--没识破 有身形
		--没在控制角色
	...
	if rela("敌对") then
		if not keydown("TURNLEFT") and not keydown("TURNRIGHT") and not keydown("JUMP") then	--没按下左转 右转 跳
			if v["识破时间"] > 0 or v["身形数量"] <= 0 then
				if dis() > 5 and gettimer("訣雲勢") > 0.3 and gettimer("决云势冲刺") > 0.3 then
					turn()		--面向目标
					movef(true)	--向前移动
				end
			end
