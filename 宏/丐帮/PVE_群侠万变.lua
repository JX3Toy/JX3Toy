--[[ 奇穴: [玄黄][坚冰][斜打狗背][无疆][凤歌][越渊][温酒][龙醒][留魄][含弘][饮江][城复于隍]
秘籍:
恶狗 1回蓝 3伤害
蜀犬 2会心 2伤害
龙腾 2会心 2伤害
...
local CastX = function(szSkill)
	if cast(szSkill) then
		if v["输出信息"] then
			print("蓝:"..format("%0.3f",mana()), "蛟影时间:"..v["蛟影时间"], "镇慑时间:"..v["镇慑时间"], "无疆时间:"..v["无疆时间"], "盈久时间:"..v["盈久时间"], "酒时间:"..v["酒时间"], "恶狗CD:"..scdtime("恶狗拦路"), "蜀犬CD:"..scdtime("蜀犬吠日"), "亢龙CD:"..scdtime("亢龙有悔"), "酒CD:"..scdtime("酒中仙"), "时乘CD:"..scdtime("时乘六龙"))
		end
		return true
	end
	return false
...
f["有酒"] = function()
	...
	--龙战
	--if gettimer("释放亢龙有悔") < 3.5 and nobuff("龙战于野") and v["镇慑时间"] < 5 then
	if mana() > 0.6 and nobuff("5989|5985|5986|5987") and nobuff("龙战于野") and v["镇慑时间"] < 5 then
		CastX("龙战于野")
	end

	--蜀犬
	if mana() < 0.85 and scdtime("恶狗拦路") > 1 and v["酒中仙CD"] > 7 then
		CastX("蜀犬吠日")
	end

	--喝酒
	if mana() >= 0.35 and nobuff("5989|5985|5986|5987") then	--没有时乘2段, 蛟龙234段
		--if scdtime("蜀犬吠日") < 17.5 and scdtime("时乘六龙") < 9.4375 then
		if scdtime("时乘六龙") < 9.4375 then
			f["酒中仙"]()
		end
	end
