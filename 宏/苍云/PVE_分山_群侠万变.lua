--[[ 奇穴: [刀魂][绝返][分野][血魄][锋鸣][割裂][业火麟光][恋战][从容][愤恨][蔑视][阵云结晦]
秘籍:
盾刀  1会心 3伤害
盾压  1调息 1会心 2伤害
劫刀  1会心 2伤害 1消耗
...
function Main(g_player)
	...
	if pose("擎刀") then
		...
		v["绝刀没快进CD"] = nobuff("8453") or bufftime("8453") > cdinterval(16) + 0.25	--buff 8453 结束, 绝刀CD
		
		--狂绝
		if buff("狂绝") or buff("8474") then
			CastX("绝刀")
		end

		--斩刀
		if nobuff("狂绝") and v["盾飞时间"] > (cdinterval(16) + 0.125) * 3 and v["怒气"] >= 25 then
			CastX("斩刀")
		end

		--闪刀, 15点怒气
		if v["没有麟光甲或已触发"] and v["绝刀没快进CD"] and nobuff("狂绝") and v["怒气"] >= 25 and tbuff("流血", id()) and tnobuff("割裂", id()) then
			CastX("闪刀")
		end

		--劫刀
		if nobuff("狂绝") and buff("麟黯") and v["绝刀没快进CD"] and bufftime("麟光玄甲") > cdinterval(16) + 0.25 and v["怒气"] >= 15 then
			CastX("劫刀")
		end

		--阵云
		if nobuff("狂绝") and v["盾飞时间"] > (cdinterval(16) + 0.125) * 3 and bufftime("麟光玄甲") < 0 then
			if v["阵云层数"] == 4 then
		...
		--阵云
		if nobuff("狂绝") and v["盾飞时间"] > (cdinterval(16) + 0.125) * 2 then
...
function PrintInfo(s)
	if s then
		print(s, "怒气:"..v["怒气"], "阵云层数:"..v["阵云层数"], "盾飞时间:"..v["盾飞时间"], "斩刀CD:"..v["斩刀CD"], "绝刀CD:"..v["绝刀CD"], "盾飞:"..cn("盾飞"), cntime("盾飞", true), "血怒:"..cn("血怒"), cntime("血怒"))
	else
		print("怒气:"..v["怒气"], "阵云层数:"..v["阵云层数"], "盾飞时间:"..v["盾飞时间"], "斩刀CD:"..v["斩刀CD"], "绝刀CD:"..v["绝刀CD"], "盾飞:"..cn("盾飞"), cntime("盾飞", true), "血怒:"..cn("血怒"), cntime("血怒"))
	end
end
