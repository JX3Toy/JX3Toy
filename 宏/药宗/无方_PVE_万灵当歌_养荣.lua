--[[ 奇穴: [淮茵][鸩羽][结草][灵荆][苦苛][坚阴][相使][凄骨][疾根][紫伏][甘遂][养荣]
秘籍:
商陆  2会心 2伤害
钩吻  2会心 2伤害
川乌  2会心 1伤害 1回内
...
function Main()
	...
	--记录读条结束
	if casting("商陆缀寒") and castleft() < 0.13 then
		settimer("商陆读条结束")
	end
	if casting("川乌射罔") and castleft() < 0.13 then
		settimer("川乌读条结束")
	end
	if casting("且待时休") and castleft() < 0.13 then
		settimer("且待读条结束")
	end
	...
	if fight() and rela("敌对") and dis() < 20 and state("站立") and cdleft(16) <= 0 and castleft() <= 0 then
		if gettimer("川乌射罔") > 0.3 and gettimer("商陆缀寒") > 0.3 and gettimer("且待时休") > 0.3 and gettimer("苍棘缚地") > 0.3 and gettimer("川乌读条结束") > 0.3 and gettimer("且待读条结束") > 0.3 then
			PrintInfo("-- 没放技能")
		end
	end
end
