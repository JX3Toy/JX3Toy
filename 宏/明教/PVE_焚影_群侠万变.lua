...
function Main(g_player)
	--减伤
	if fight() and life() < 0.4 and nobuff("贪魔体") then
		cast("贪魔体")
	end
	...
	v["日月灵魂时间"] = bufftime("日月灵魂")
	v["日月灵魂层数"] = buffsn("日月灵魂")
	v["日月同辉时间"] = bufftime("日月同辉")
	v["日月同辉层数"] = buffsn("日月同辉")
	...
	--目标是敌对, 4尺内, 没打技能输出信息
	if fight() and rela("敌对") and dis() < 4 and cdleft(503) <= 0 and nobuff("贪魔体") then
		PrintInfo("----------没打技能, ")
	end
end
...
function PrintInfo(s)
	...
	if s then
		print(s, "日灵:"..v["日灵"], "月魂:"..v["月魂"], "满日:"..szSun, "满月:"..szMoon, "明光日:"..bufftime("25758"), "日月灵魂:"..v["日月灵魂层数"], v["日月灵魂时间"], "日月齐光:"..v["日月齐光等级"], v["日月齐光时间"], "生死劫CD:"..v["生死劫CD"], "隐身CD:"..v["隐身CD"], "日月同辉:"..v["日月同辉层数"], v["日月同辉时间"], "日斩CD:"..scdtime("烈日斩"), "月斩CD:"..scdtime("银月斩"))
	else
		print("日灵:"..v["日灵"], "月魂:"..v["月魂"], "满日:"..szSun, "满月:"..szMoon, "明光日:"..bufftime("25758"), "日月灵魂:"..v["日月灵魂层数"], v["日月灵魂时间"], "日月齐光:"..v["日月齐光等级"], v["日月齐光时间"], "生死劫CD:"..v["生死劫CD"], "隐身CD:"..v["隐身CD"], "日月同辉:"..v["日月同辉层数"], v["日月同辉时间"], "日斩CD:"..scdtime("烈日斩"), "月斩CD:"..scdtime("银月斩"))
	end
end
...
function DPS()
	--没进战先隐身
	if nofight() and rela("敌对") and dis() < 20 and nobuff("暗尘弥散|4908|11547|12492") then
		CastX("暗尘弥散")
	end
	...
	--有日悬
	if buff("悬象著明·日") then
		if bufftime("诛邪镇魔") <= v["GCD时间"] + 0.125 then
			CastX("诛邪镇魔")
		end
		CastX("生死劫")
		CastX("诛邪镇魔")		--12尺正面
	...
	--诛邪时间快到了
	if bufftime("诛邪镇魔") < v["GCD时间"] + 0.25 then
		CastX("诛邪镇魔")
	end
	...
	if v["隐身CD"] > 13 then
		...
		if v["日月齐光时间"] > 0 or (v["满日"] and bufftime("日月齐光·月") > 0) or (v["满月"] and bufftime("日月齐光·日") > 0) then
			CastX("生死劫")
		end
	end
	...
	--诛邪
	if scdtime("烈日斩") > 0 and scdtime("银月斩") > 0 then		--日月斩还在冷却
		CastX("诛邪镇魔")
	end
	...
	--破魔
	if v["满日"] and v["隐身CD"] < 8 and cdtime("净世破魔击") <= 0 then
		if cbuff("25721") then
			print("取消日月齐光，准备进悬象")
		end
	end
