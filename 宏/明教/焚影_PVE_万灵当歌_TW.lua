...
function Main()
	...
	--减伤
	if fight() and life() < 0.5 and nobuff("貪魔體") then
		cast("貪魔體")
	end
	...
	if nofight() and nobuff("暗塵彌散|4908|11547|12492") then
		--隐身回灵
	...
	--没打技能记录信息
	if fight() and rela("敌对") and dis() < 4 and cdleft(503) <= 0 and nobuff("貪魔體") and state("站立") then
		PrintInfo("----------没打技能")
	end
end
...
f["悬象阶段"] = function()
	...
	if CastX("幽月輪") then		--正常应该不会打, 加上防止卡在悬象阶段
		print("------------------------------ 悬象中打幽月轮")
	end
end
...
f["正常循环"] = function()
			...
			if (v["日月灵魂时间"] > 0 and v["日月灵魂层数"] >= 2)  or v["日月同辉时间"] > (v["GCD间隔"] + 0.25) * 3 then
				if v["悬象CD"] < v["GCD间隔"] and v["暗尘CD"] < v["GCD间隔"] then
					if rela("敌对") and dis() < 4 and face() < 80 then
						if cdtime("懸象著明") <= 0 and cdtime("暗塵彌散") <= 0 then
							CastX("暗塵彌散")
							CastX("懸象著明")
						end
					end
	...
	if buff("靈·日|魂·月") and v["暗尘CD"] > 13 then	--破魔后
		CastX("生死劫")
	end

	--净世破魔击
	if v["满日"] and cdtime("淨世破魔擊") <= 0 and rela("敌对") and dis() < 15 then	
		if v["暗尘CD"] < 8 then		--这个时间有待测试
			if cbuff("日月齊光·壹") then
				print("---------- 取消齐光一")
			end
		end
	end
...
f["起手隐身驱夜"] = function()
	if buff("12850") and nobuff("日月同輝") then
		CastX("驅夜斷愁")
	end
end
...
function CastX(szSkill, bSelf)
	if cast(szSkill, bSelf) then
		...
		if v["记录信息"] then
			PrintInfo()
			if szSkill == "淨世破魔擊" then
				if v["满日"] then
					print("-------------------- 日破")
				else
					print("-------------------- 月破")
				end
				if cdtime("暗塵彌散") <= 0 then
					print("---------------------------------------- 暗尘冷却")
				end
			end
			if szSkill == "生死劫" then
...
[光明相] 必会心对生死劫dot有效
[明光恆照] 对生死劫dot有效
[明光恆照] 有Bug, 要打过满日有buff 25758(满月加伤害) 打满月才给buff 25759(满日加会心), 没有 25758 打满月不给 25759
[淨體不畏] 伤害部分, 判断目标烈日时谁上的都行, 银月斩要自己上的才有效果, 没什么影响反正银月斩都要打
[日月齊光] 3种类型, 1破魔 buff 25928, 2生死劫 buff 25929, 3悬象 buff 25930, 有表示没打过
--]]
