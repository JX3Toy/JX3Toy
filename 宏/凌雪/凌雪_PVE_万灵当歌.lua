--[[ 奇穴: [秋霁][雪覆][折意][风骨][北阙][渊岳][玄肃][飞刃回转][百节][忘断][徵逐][孤路]
秘籍:
星垂	1会心 3伤害
金戈	2调息 2伤害
寂洪荒	3伤害 1消耗
...
function Main()
	...
	if casting("乱天狼") and castleft() < 0.13 then
		settimer("乱天狼读条结束")
	end
	...
	v["斩无常CD"] = scdtime("斩无常")
	v["血覆充能次数"] = cn("血覆黄泉")
	v["血覆充能时间"] = cntime("血覆黄泉", true)
	v["日月CD"] = scdtime("日月吴钩")
	v["崔嵬CD"] = scdtime("崔嵬鬼步")
	v["飞刃CD"] = scdtime("飞刃回转")
	v["寂洪荒读条时间"] = casttime("寂洪荒")
	v["乱天狼读条时间"] = casttime("乱天狼")
	v["斩无常读条时间"] = casttime("斩无常")

	v["目标链接时间"] = tbufftime("链接", id())
	...
	v["百节层数"] = buffsn("百节")			--6秒 3层 实际效果 15927 15928 15929, 加寂乱隐斩4个技能伤害 10% 20% 30%
	v["百节时间"] = bufftime("百节")
	v["忘断时间"] = bufftime("忘断")		--10秒, 攻击+25%
	...
	if buff("单链") or gettimer("血覆黄泉") < 0.3 then
		if gettimer("幽冥窥月") > 0.3 then
			f["单链"]()
		end
	end

	if nobuff("单链") or gettimer("幽冥窥月") < 0.3 then
		if gettimer("血覆黄泉") > 0.3 then
			f["双持"]()
		end
	end

	--没放技能记录信息
	if fight() and rela("敌对") and dis() < 12 and face() < 60 and cdleft(16) <= 0 and castleft() <= 0  and state("站立|走路|跑步|跳跃") then
		if gettimer("幽冥窥月") > 0.3 and gettimer("血覆黄泉") > 0.3 and gettimer("寂洪荒") > 0.3 and gettimer("寂洪荒读条结束") > 0.3 and gettimer("乱天狼") > 0.3 and gettimer("乱天狼读条结束") > 0.3 and gettimer("斩无常") > 0.3 then
			PrintInfo("---------- 没放技能")
		end
	end
end
...
f["单链"] = function()
	...
	if not v["移动键被按下"] and dis() < 4 and cdtime("斩无常") <= 0 then
...
f["双持"] = function()
	...
	--乱天狼
	if v["放乱天狼"] then
		if v["徵逐时间"] > v["乱天狼读条时间"] or gettimer("幽冥窥月") < 0.3 then
			aCastX("乱天狼")
		end
		if v["徵逐时间"] <= v["乱天狼读条时间"] and gettimer("幽冥窥月") > 0.3 then
			f["出链"]("乱天狼续徵逐")
		end
	end
...
f["出链"] = function(szReason)
	if gettimer("血覆黄泉") > 0.3 and CastX("血覆黄泉") then
...
f["收链"] = function(szReason)
	if gettimer("幽冥窥月") > 0.3 and CastX("幽冥窥月") then
