...
推荐1级加速, 无法经常保持10尺外的场景 [秋風散影] 换成 [奪魄之威]
--]]
...
function Main(g_player)
	...
	v["心无时间"] = bufftime("心無旁騖")
	v["注能标记"] = bufflv("26055")			--标记上次打过的注能技能
	v["命陨时间"] = bufftime("命隕")		--[千里无痕] 暴雨完整读条, 攻击+20% 15秒
	v["白雨层数"] = buffsn("白雨跳珠")
	...
	--橙武buff
	if buff("3487") and tbuffsn("星斗闌干", id()) < 3 then
		CastX("逐星箭")
	end

	--穿心弩 保dot
	if v["目标穿心时间"] <= casttime("暴雨梨花針") + v["GCD间隔"] + 0.5 then
		f["穿心弩"]()
	end
	...
	--追命箭
	if buff("追命無聲") and v["白雨时间"] >= 0 and v["注能标记"] ~= 1 then
		CastX("追命箭")
	end
	...
	if not v["移动键被按下"] and v["注能标记"] ~= 6 then
		...
		if v["目标血量较多"] and dis() < 23 then
			if cdtime("暴雨梨花針") <= 0 and v["神机值"] >= 50 and v["百里CD"] < 16 then	--暴雨前
				CastX("心無旁騖")
			end
		end
		CastX("暴雨梨花針")
	...
	--逐星穿心竞争白雨1层
	if v["追命CD"] > v["GCD间隔"] + 1 + casttime("暴雨梨花針") and v["无声层数"] == 0 then
		f["逐星箭"]()
	end
	...
	if bufftime("追命無聲") > 1 and v["白雨时间"] > 1 and v["追命CD"] < 0.5 then
		--等追命
	else
		f["逐星箭"]()
	end
	...
	if fight() and dis() < 25 and state("站立") and cdleft(16) <= 0 and castleft() <= 0 and gettimer("穿心弩") > 0.3 and gettimer("暴雨梨花針") > 0.3 and gettimer(18672) > 0.3 and gettimer("百里读条结束") > 0.3 then
		PrintInfo("----------没放技能")
	end
end
