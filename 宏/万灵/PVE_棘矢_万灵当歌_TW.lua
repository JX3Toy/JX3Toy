--[[ 奇穴: [素矰][棘矢][襄尺][長右][鹿蜀][桑柘][於狩][盧令][託月][佩弦][貫侯][朝儀萬匯]
秘籍:
劲风	2会心 2伤害
饮羽	2调息 1重置 1非侠
引风	3调息 1回血
...
--主循环
function Main()
	if casting("飲羽簇") and castleft() < 0.13 then
		settimer("饮羽簇读条结束")
	end
	...
	v["承契层数"] = buffsn("承契")
	v["承契时间"] = bufftime("承契")
	v["贯穿层数"] = tbuffsn("貫穿", id())
	v["贯穿时间"] = tbufftime("貫穿", id())
	v["标鹄层数"] = tbuffsn("標鵠")
	v["标鹄时间"] = tbufftime("標鵠", id())
	...
	--[[
	if fight() and rela("敌对") and dis() < 25 and cdleft(16) <= 0 and castleft() <= 0 and gettimer("朝儀萬匯") > 0.5 and gettimer("弛風鳴角") > 0.5 then
		PrintInfo("---------- 没打技能")
	end
	--]]
end
