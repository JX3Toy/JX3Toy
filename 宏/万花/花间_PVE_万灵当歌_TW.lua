--[[ 奇穴: [彈指][青冠][倚天][踏歌][青歌][雪中行][清流][鍾靈][流離][雪棄][焚玉][涓流]
秘籍:
星楼  2调息 1消耗 1减伤
阳明  2伤害 2会心
商阳  1持续 1距离 2消耗
...
function Main(g_player)
	...
	--等待阳明指读条
	if casting("陽明指") and castleft() < 0.13 then
		settimer("陽明指读条结束")
	end
	if gettimer("陽明指读条结束") < 0.3 then
		return
	end

	--钟林读条结束记录目标
	if casting("鐘林毓秀") and castleft() < 0.13 then
		v["钟林目标"] = casttarget()
		settimer("钟林读条结束")
	end

	--兰摧读条结束记录目标
	if casting("蘭摧玉折") and castleft() < 0.13 then
		v["兰摧目标"] = casttarget()
		settimer("兰摧读条结束")
	end

	--初始化变量
	v["目标钟林时间"] = tbufftime("鐘林毓秀", id())
	v["目标商阳时间"] = tbufftime("商陽指", id())
	v["目标兰摧时间"] = tbufftime("蘭摧玉折", id())
	v["目标快雪时间"] = tbufftime("快雪時晴", id())
	v["目标有钟林"] = f["目标有钟林"]()
	v["目标有商阳"] = f["目标有商阳"]()
	v["目标有兰摧"] = f["目标有兰摧"]()
	v["目标有快雪"] = f["目标有快雪"]()
	...
	--兰摧
	if buff("亂灑青荷") and not v["乱撒后放过阳明指"] then
	...
	--快雪
	if not v["目标有快雪"] or buffsn("濺玉") < 3 then
		CastX("快雪時晴")
	end

	--阳明指
	if buff("亂灑青荷") and not v["乱撒后放过阳明指"] then	--乱撒后阳明指上兰摧钟林
		CastX("陽明指")
	end
	...
	--[[测试
	if casting("快雪時晴") and castleft() < 0.13 then
		settimer("快雪读条结束")
	end
	if rela("敌对") and dis() < 20 and cdleft(16) <= 0 and castleft() <= 0 and gettimer("快雪時晴") > 0.3 and gettimer("鐘林毓秀") > 0.3 and gettimer("蘭摧玉折") > 0.3 and gettimer("快雪读条结束") > 0.3 then
		PrintInfo("---------- 没打技能")
	end
	--]]
end
