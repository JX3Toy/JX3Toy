...
function Main(g_player)
	...
	--水月无间
	if rela("敌对") and dis() < 18 and mana() < 0.9 then
		cast("水月无间")
	end
	...
	--乱洒青荷
	if rela("敌对") and dis() < 18 and v["商阳指时间"] <= 0 and v["钟林时间"] <= 0 and v["兰摧时间"] <= 0 then
		if cdtime("玉石俱焚") > 15 then
			cast("乱洒青荷")
		end
	end
	...
	if gettimer("吞海") > 2 and v["有3dot"] then
		...
		--芙蓉并蒂刷新dot
		if scdtime("玉石俱焚") < 1 and v["最短时间"] > 0.1 and v["最短时间"] < 10 then
			cast("芙蓉并蒂")
		end
	end
