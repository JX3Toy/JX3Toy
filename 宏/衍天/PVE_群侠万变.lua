--[[ 奇穴: [水盈][天网][顺祝][列宿游][重山][堪卜][亘天][连断][荧入白][征凶][焚如][增卜]
秘籍:
三星临		1会心 3伤害
兵主逆		1会心 3伤害
天斗旋		1调息 1会心 2伤害
...
function Main(g_player)
	...
	v["荧入白时间"] = bufftime("荧入白")	--兵每跳延长 卦象 主动 变卦 荧入白
	v["卦象"] = "无卦"
	v["卦象时间"] = -100
	local tGX = { "水坎", "山艮", "火离" }
	...
	v["起卦CD"] = scdtime("起卦")
	v["变卦充能次数"] = cn("变卦")
	v["变卦充能时间"] = cntime("变卦", true)

	local tSpeedXY, tSpeedZ = speed(tid())
	v["目标静止"] = rela("敌对") and tSpeedXY <= 0 and tSpeedZ <= 0

	---------------------------------------------等待buff同步
	if gettimer("奇门飞宫") < 0.3 or gettimer("返闭惊魂") < 0.3 then
		print("----------等待灯同步")
		return
	end
	if gettimer("起卦") < 0.3 or gettimer("变卦") < 0.3 or gettimer("祝由·火离") < 0.3 then
		print("----------等待卦象同步")
		return
	end
	...
	if rela("敌对") and v["灯数量"] < 3 then
		if CastX("奇门飞宫") then return end
	end

	--重置过放新灯
	if rela("敌对") and buff("24656") and gettimer("奇门飞宫") >= 0.5 and gettimer("释放奇门飞宫") > 4 then
		if CastX("奇门飞宫") then return end
	end

	--返闭放灯
	if rela("敌对") and dis() < 20 and v["灯数量"] < 3 then
		if CastX("返闭惊魂") then return end
	end
	
	--返闭续时间
	if v["灯数量"] >= 3 and v["连局时间"] < 3.5 then
		if nobuff("24656") and gettimer("列宿游") >= 0.5 and v["灯1层数"] >= 3 and v["灯2层数"] >= 3 and v["灯3层数"] >= 3 then
			if CastX("返闭惊魂") then return end
		end
	end

	--移灯
	if v["目标静止"] and dis() < 20 and gettimer("奇门飞宫") >= 0.5 then
		if v["灯数量"] >= 3 or cn("奇门飞宫") < 1 then	--有3个灯或者放不出灯
	...
	---------------------------------------------处理卦象
	v["目标没火离"] = false
	if rela("敌对") and dis() < 25 and v["目标火离时间"] < 0 and gettimer("祝由·火离") >= 0.5 then
		v["目标没火离"] = true
	end
	...
	if v["火离时间"] >= 0 then
		...
		if gettimer("祝由·火离") > 0.3 then
			...
			if bCast then
				if CastX("祝由·火离") then return end
			end
		end
	...
	if fight() then
		...
		elseif v["水坎时间"] >= 0 then
			v["需要起卦"] = "水卦直接起"
		elseif v["火离时间"] >= 0 and nobuff("17825") then
			v["需要起卦"] = "火卦主动打过了"
	...
	--if fight() and rela("敌对") and dis() < 20 and face() < 90 and cdleft(16) <= 0 and castleft() <= 0 and gettimer("天斗旋") > 0.3 and state("站立|走路|跑步|跳跃") then
	--	PrintInfo("----------没打技能, ")
	--end
end
