...
f["处理灯"] = function()
	--放灯
	if v["灯数量"] < 3 then
		if v["灯数量"] < 2 or gettimer("返闭惊魂") > 0.3 then
			CastX("奇门飞宫")
		end
		if v["灯数量"] < 2 or gettimer("奇门飞宫") > 0.3 then
			CastX("返闭惊魂")
		end
	end

	--重置过放新灯
	if buff("24656") and gettimer("奇门飞宫") > 0.3 and gettimer("奇门飞宫") > 0.3 and gettimer("生成魂灯") > 5 then
		CastX("奇门飞宫")
	end
	...
	--移灯
	if v["目标静止"] and dis() < 20 and gettimer("奇门飞宫") > 0.3 and gettimer("生成魂灯") > 0.3 then
