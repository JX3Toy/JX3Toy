--[[ 奇穴: [冰體][蠱夢][連契][蟲獸][翩翾][淵兮][納精][訣靈][仙王蠱鼎][蝶隱][五色][迷仙引夢]
秘籍:
凤凰蛊  2调息 2效果
献祭  2调息 2效果
冰蚕  3效果 1读条
...
function Main()
	...
	if casting("仙王蠱鼎") and castleft() < 0.13 then
		settimer("鼎读条结束")
	end
	...
	if fight() and v["目标是boss"] and ttid() ~= 0 and ttid() ~= id() then	--目标是boss 目标的目标存在 不是自己
		if xbufftime("蠱惑眾生", ttid(), id()) < 5 then
			xcast("蠱惑眾生", ttid())	--给目标的目标
		end
	end
	...
	--吃鼎
	if gettimer("鼎读条结束") > 0.5 and nobuff("蠱時") and life() < 0.7 and cdleft(16) > 0.5 then
		interact("仙王蠱鼎")
	end
