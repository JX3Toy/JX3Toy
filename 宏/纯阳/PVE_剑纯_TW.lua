--[[ 奇穴:[心固][環月][化三清][鏡花影][風逝][疊刃][長生][裂雲][故長][期聲][虛極][玄門]
秘籍:
生太极	3读条 1效果
吞日月	全点
...
function Main()
	...
	v["八荒CD"] = scdtime("八荒歸元")
	v["人剑CD"] = scdtime("人劍合一")
	v["叠刃时间"] = tbufftime("疊刃", id())
	v["叠刃层数"] = tbuffsn("疊刃", id())
	v["风逝时间"] = bufftime("風逝")
	v["玄门时间"] = bufftime("玄門")
	v["玄门层数"] = buffsn("玄門")
	...
	v["化三清"] = npc("关系:自己", "名字:氣場化三清", "距离<15")
	v["碎星辰"] = npc("关系:自己", "名字:氣場碎星辰", "距离<13")
	v["吞日月"] = npc("关系:自己", "名字:氣場吞日月", "距离<13")
	_, v["气场数量"] = npc("关系:自己", "名字:氣場生太極|氣場碎星辰|氣場吞日月", "距离<13")
	...
	--减伤
	if fight() and life() < 0.5 and gettimer("坐忘無我") > 0.3 and nobuff("坐忘無我") and gettimer("轉乾坤") > 0.3 and buffstate("减伤效果") < 40 then
		if CastX("坐忘無我") then return end
		if CastX("轉乾坤") then return end
	end

	--没进战 3碎
	if nofight() and v["目标静止"] then
		_, v["目标气场数量"] = tnpc("关系:自己", "名字:氣場生太極|氣場碎星辰|氣場吞日月", "距离<13")
		if v["目标气场数量"] < 3 then
			CastX("碎星辰")
		end
	end
	...
	--碎星辰
	v["碎星辰距离"], v["碎星辰时间"] = qc("氣場碎星辰", id(), id())
	...
	if not getopt("手动爆发") or keydown(1) then	--没打开手动爆发选项, 或者按下快捷键1(不是按键1, 是在快捷键设置中自己设置的快捷键1对应的按键)
		if nobuff("紫氣東來") and rela("敌对") and dis() < 6 and face() < 90 and bufftime("鏡花影") > 0.5 and cdtime("鏡花影") <= 0 then
	...
	--万剑归宗
	if rela("敌对") and dis() < 8 and tnpc("关系:自己", "名字:氣場吞日月", "角色距离<10") ~= 0 then
		CastX("萬劍歸宗")
	end
	...
	--没打技能输出信息
	if fight() and rela("敌对") and dis() < 6 and cdleft(16) <= 0 and castleft() <= 0 and gettimer("吞日月") > 0.3 and gettimer("碎星辰") > 0.3 and gettimer("生太極") > 0.3 and gettimer("化三清") > 0.3 and state("站立|走路|跑步|跳躍") then
		PrintInfo("----------没打技能, ")
	end
end
