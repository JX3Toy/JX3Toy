--[[ 奇穴: [青梅嗅][千里冰封][新妆][青梅][枕上][广陵月][流玉][钗燕][盈袖][化冰][夜天][凝华]
秘籍:
玳弦  3伤害 1距离
...
function Main(g_player)
	...
	v["钗燕次数"] = buffsn("钗燕")	--26287, 6秒, 层数是次数, 结束时删除所有buff
	...
	--琼宵
	if qixue("琼宵") and v["目标静止"] and dis() < 9 then
		v["移动键被按下"] = keydown("MOVEFORWARD") or keydown("MOVEBACKWARD") or keydown("STRAFELEFT") or keydown("STRAFERIGHT") or keydown("JUMP")
		if not v["移动键被按下"] and state("站立") and nobuff("剑神无我") and v["钗燕没玳弦"] and v["钗燕没剑影"] and v["钗燕没剑气"] and v["钗燕没剑破"] and v["钗燕没江海"] then
			CastX("婆罗门")
		end
	end
