...
function Main(g_player)
	...
	-------------------------------------------------------地面
		...
		--定波快充能结束
		if cntime("定波砥澜", true) < cdinterval(16) and v["跃潮CD"] <= 0 then
		...
		--定波快充能结束
		if cntime("定波砥澜", true) < cdinterval(16) and v["海运CD"] > 0 then
			if gettimer("定波读条结束") > 0.5 and v["目标静止"] and dis() < 5 then
				if CastX("定波砥澜") then
					settimer("定波砥澜")
				end
		...
		--木落
		if (nobuff("太息") and (gettimer("定波砥澜") < 3 or cn("定波砥澜") < 1)) or tbuff("惊潮", id()) then
			CastX("木落雁归")
		end
